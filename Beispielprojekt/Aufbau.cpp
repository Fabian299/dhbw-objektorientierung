...
/*
void Settings::set_rankinglist(const int Userscore, char Username) {
	...
	if (Userscore > this->TopTenRanking[9][1]){	// Name , Score
		...
		for(int i = 8; i>=0; i--){
			if(Userscore<this->TopTenRanking[i]) {     
				...
				this->TopTenPlayer[i+1]=Username;  //geht das?
				i=0;
...
int Settings::get_rankinglist(){ 
	return this->TopTenPlayer, this->TopTenRanking;
} 

int Settings::print_rankinglist(){
	int TopTenNames[10];
	int TopTenScores[10];
	TopTenNames, TopTenScores = get_rankinglist();
   	for (int i = 0; i < 10; i++) {
	   cout << "Name: " << TopTenNames[i] << "\tScore: " << TopTenScores[i] << endl;
	}	
}
*/
// /*
...
// 	if (eingabe == '2') ablauf_changename();
...
// 	cout << "Press 'L' to use Luigi or 'M' to use Mario" << endl;

// 	char eingabe = cin.get();
// 	if (eingabe == '1') fkt_reset_rankinglist();
// 	if (eingabe == '2') fkt_change_character();
// 	if (eingabe == '3') ablauf_mainmenue();
// 	if (eingabe == 'l' || eingabe == 'L') character = Luigi; // Luigi
// 	if (eingabe == 'm' || eingabe == 'M') character = Mario; // Mario
// 	else ablauf_setting();
